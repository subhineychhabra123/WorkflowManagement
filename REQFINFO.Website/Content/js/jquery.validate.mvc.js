...
; (function ($, window, document, undefined) {
    ...
    function Plugin(element, options) {
        ...
        this.dataMsgFor = this.options.dataMsgFor == undefined ? '' : this.options.dataMsgFor;
        this.init();
    }
    ...
    Plugin.prototype = {
        ...
        init: function () {
            ...
            if (name || this.dataMsgFor != '') {
                var errValElem;
                if (this.dataMsgFor != '') {
                    errValElem = $('span[data-valmsg-for="' + this.dataMsgFor + '"]');
                }
                else {
                    errValElem = $('span[data-valmsg-for="' + name + '"]');
                }
                ...
                function onDOMSubtreeModified(errValElem) {
                    ...
                    errValElem.bind("DOMSubtreeModified", function () {
                        ...
                        if (errValElem.hasClass("field-validation-error")) {
                            elem.css('border', '1px solid #f3797f');
                        }
                        else {
                ...
                if (elem.is('select')) { //Handle tooltip on input type = select (DropdownList)
                ...
                else if (elem.attr('type') == 'date' || elem.attr('type') == 'button' || elem[0].className == 'parentNgInputTag' || elem[0].className == 'fileupload fileupload-new form1' || elem[0].className == 'col-lg-4 col-md-4 col-sm-4 col-xs-12') { //Handle if input type = date
                    elem.on("focus mouseenter mouseover", function (event) {
                        showTooltipOfError(event);
                    });
                    elem.on("mouseleave", function (event) {
                    ...
                    elem.on("focus mouseover", function (event) {
                        showTooltipOfError(event);
                    });
